...
class DashboardController extends Controller
{
    ...
    private function doctorStats(User $user): array
    {
        ...
        // Optional fallback for testing: if no appointments today, pull the doctor's most recent ones so the UI isn't blank
        if ($schedule->isEmpty()) {
            $schedule = Appointment::with(['patient:id,code,name', 'doctor.user:id,name'])
                ->where('doctor_id', $doctorId)
    ...
    private function receptionistStats(): array
    {
        ...
        // Fallback if no appointments are set for today
        if ($schedule->isEmpty()) {
            $schedule = Appointment::with(['patient:id,code,name', 'doctor.user:id,name'])
                ->latest('appointment_date')
                ->limit(5)
        ...
        return [
            'patients' => Patient::count(),
            'appointments_today' => Appointment::whereDate('appointment_date', today())->whereNotIn('status', ['cancelled'])->count(),
            'pending_appointments' => Appointment::where('status', 'pending')->count(),
            'current_admissions' => Admission::where('status', 'admitted')->count(),
            'unpaid_bills' => Bill::whereIn('status', ['unpaid', 'partially_paid'])->count(),
            'todays_schedule' => $schedule,
            'todays_appointments' => $schedule,
        ];
    }
