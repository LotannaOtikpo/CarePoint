...
const EMPTY = {
  ...
  address: '', emergency_contact_name: '', emergency_contact_phone: '', password: '',
};
...
export default function Patients() {
  ...
  const { rows, page, setPage, lastPage, reload, loading } = useList('/patients', { search });
  ...
  const open = (p) => {
    setError('');
    setEditing(p ?? 'new');
    setForm(p ? { ...EMPTY, ...p, dob: p.dob?.slice(0, 10) ?? '', password: '' } : EMPTY);
  };

  const save = async (e) => {
    e.preventDefault();
    try {
      // If password field is empty on edit, omit it so it doesn't overwrite with blank
      const payload = { ...form };
      if (editing !== 'new' && !payload.password) {
        delete payload.password;
      }

      if (editing === 'new') await client.post('/patients', payload);
      else await client.put(`/patients/${editing.id}`, payload);
        ...
        <Modal title={editing === 'new' ? 'Add Patient' : 'Edit Patient'} onClose={() => setEditing(null)}>
          <form onSubmit={save}>
            <div className="form-grid">
              ...
              {isAdmin && (
                <div className="full">
                  <label>Password {editing === 'new' ? '*' : '(leave blank to keep current)'}</label>
                  <input 
                    type="password" 
                    value={form.password || ''} 
