...
const metricValue = (key, value) => {
  ...
  return Number.isFinite(Number(value)) ? Number(value).toLocaleString('en-NG') : value;
};
...
export default function Dashboard() {
  ...
  useEffect(() => {
    client.get('/dashboard/stats')
      ...
      .catch(() => setError('CarePoint metrics could not be loaded. Please refresh to try again.'));
  }, []);

  if (error) {
    return <div className="empty-state"><Activity size={24} /><h2>Unable to load CarePoint overview</h2><p>{error}</p></div>;
  }
  ...
  return (
    <>
      <div className="page-head dashboard-head">
        <div>
          <span className="page-eyebrow">CarePoint clinical overview</span>
          <h2>{greeting}</h2>
          <p className="page-description">A real-time view of today’s clinical operations and patient priorities.</p>
        </div>
