...
export default function Billing() {
  ...
  const { rows, page, setPage, lastPage, reload, loading } = useList('/bills', { status });

  const [adding, setAdding] = useState(false);
  const [editingStatusBill, setEditingStatusBill] = useState(null);
  ...
  const saveStatus = async (e) => {
    ...
    setError('');
    try {
      await client.put(`/bills/${editingStatusBill.id}/status`, { status: newStatus });
      setEditingStatusBill(null);
      reload();
    } catch (err) {
  ...
  return (
    <>
      <div className="page-head">
        <h2>Billing</h2>
        {canEdit && <button className="btn" onClick={openAdd}>+ New Bill</button>}
      </div>
      ...
      <div className="table-wrapper">
        <table>
          <thead>
            <tr><th>Bill No.</th><th>Patient</th><th>Total</th><th>Paid</th><th>Status</th>{(canEdit || isAdmin) && <th></th>}</tr>
          </thead>
          <tbody>
            {loading ? (
              <tr>
                    ...
                    {(canEdit || isAdmin) && (
                      <td>
                        <div style={{ display: 'flex', gap: '4px', alignItems: 'center', flexWrap: 'wrap' }}>
                          {['unpaid', 'partially_paid'].includes(b.status) && canEdit && (
                          ...
                          {isAdmin && (
                            <>
                              <button className="btn secondary small" onClick={() => openStatusEdit(b)}>Edit Status</button>
      ...
      {editingStatusBill && (
        <Modal title={`Edit Status — ${editingStatusBill.bill_no}`} onClose={() => setEditingStatusBill(null)}>
          <form onSubmit={saveStatus}>
            <div className="form-grid">
              <div className="full">
                <label>Status *</label>
                <select value={newStatus} required onChange={(e) => setNewStatus(e.target.value)}>
                  {['unpaid', 'partially_paid', 'paid', 'cancelled'].map((s) => (
                    <option key={s} value={s}>{s.replace('_', ' ')}</option>
                  ))}
                </select>
            ...
            {error && <p className="error">{error}</p>}
            <div className="form-actions">
              <button type="button" className="btn secondary" onClick={() => setEditingStatusBill(null)}>Cancel</button>
